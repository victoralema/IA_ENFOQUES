...
class Graph:
    ...
    def backjumping_search(self, current_node, target_node, visited=[]):
        ...
        for neighbor in self.graph[current_node]:  # Itera sobre los vecinos del nodo actual
            if neighbor not in visited:  # Verifica si el vecino no ha sido visitado
                result = self.backjumping_search(neighbor, target_node, visited)  # Búsqueda recursiva desde el vecino
                if result is not None:
                    return result  # Devuelve el camino encontrado

        visited.pop()  # Desmarca el nodo actual si no conduce a la solución
        return None  # Indica que no se encontró un camino válido desde el nodo actual
...
if __name__ == "__main__":
    ...
    # Resolver el problema utilizando búsqueda con Salto Atrás Dirigido por Conflictos
    path = graph.backjumping_search(start_node, goal_node)
